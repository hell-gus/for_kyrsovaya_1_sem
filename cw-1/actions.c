...
//wchar_t* input_line(size_t size){
...
//func3
int compare_by_latin_letters(const void* arg1, const void* arg2){    Word word1 = *((Word*) arg1);    Word word2 = *((Word*) arg2);    if (word1.count_of_latin_letters > word2.count_of_latin_letters) return 1;
    if (word1.count_of_latin_letters < word2.count_of_latin_letters) return -1;    return 0;}
void sort_by_latin_letters(Text text){    for (unsigned i = 0; i < text.len; i++){        wchar_t seps[text.sents[i].len];
        for (unsigned j = 0; j < text.sents[i].len; j++){            text.sents[i].words[j].count_of_latin_letters = 0;            seps[j] = text.sents[i].words[j].sep;            for (unsigned k = 0; k < text.sents[i].words[j].len; k++){
                wchar_t symbol = text.sents[i].words[j].chars[k];                                if (wcschr(L"aeiouyAEIOUYауоиэыяюеёАУОИЭЫЯЮЕЁ", symbol) != NULL)
                    text.sents[i].words[j].count_of_latin_letters++;            }        }        qsort(text.sents[i].words, text.sents[i].len, sizeof(Word), compare_by_latin_letters);
        for (unsigned j = 0; j < text.sents[i].len; j++){            text.sents[i].words[j].sep = seps[j];        }    }
}
